...
export function HeroSection() {
  return (
    <section
      className="grid items-center gap-8 md:grid-cols-[1.15fr_1fr] w-full max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-0 md:py-24"
      aria-labelledby="hero-heading"
    >
      <motion.div
        ...
        variants={containerVariants}
      >
        {/* Badge — tight to heading, they read as one unit */}
        <div className="inline-flex w-fit items-center gap-2 rounded-full bg-surface/80 px-3 py-1 text-xs font-medium text-muted shadow-sm ring-1 ring-black/5">
          <span className="inline-flex h-1.5 w-1.5 rounded-full bg-primary" />
          AI-powered smart education for British curriculum
        </div>
        ...
        {/* Subheading — mt-5 (moderate), clear separation after headline impact */}
        <p className="mt-5 max-w-[460px] text-[16px] leading-[1.78] tracking-[0.004em] text-muted sm:text-[17px]">
          Empowering Schools, Teachers, and Students through a Smart AI
          Platform built for unlocking academic excellence and future-ready
          learners.
        </p>

        {/* CTA — mt-8 (most generous), intentional pause before action */}
      ...
      {/* Image */}
      <motion.div
        className="relative mx-auto flex w-full max-w-[420px] items-start justify-center pt-1"
        initial="hidden"
        animate="visible"
        variants={imageVariants}
      >
